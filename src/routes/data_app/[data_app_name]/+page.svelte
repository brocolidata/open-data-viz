<script>
	...
	import { updateDashboardsInDataApp } from "$lib/utils/stores";

    const dataAppName = $page.params.data_app_name;
    // let dataApp = $derived(getDataAppByName(dataAppName));
    // let dashboards = $derived(getDashboardsForDataApp(dataApp?.dashboards));
	let dataApp = $state({});
...
<div class="flex items-center mb-4">
	<h2 class="text-3xl font-bold tracking-tight">{dataApp.label}</h2>
	<DropdownMenu.Root>
	...
	/>
	<Dialog.Root 
		bind:open={openDashboardSelectDialog}
	>
		<Dialog.Content>
			<Dialog.Header>
			...
			<Select.Root bind:value={selectedDashboards} type="multiple">
				<Select.Trigger class="w-[180px]">Select Dashboards</Select.Trigger>
				<Select.Content>
					{#each $dashboardsIndex as dashboard}
						<Select.Item value={dashboard.name}>{dashboard.label}</Select.Item>		
					{/each}
				</Select.Content>
			</Select.Root>	
